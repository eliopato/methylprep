...
def check_array_folders(data_dir):
    ...
    Should deal with GEO multi-array packages. beta_bake reliably splits IDATs by array type for you, as of v1.5.6."""
    instructions = []
    ...
    if isinstance(sample_sheet_file_path, list):
        ...
        for sample_sheet_file in sample_sheet_file_path:
            if any([array_folder in sample_sheet_file.parts for array_folder in folders]):
                ...
                idats_found = (list(Path(sample_sheet_file.parent).rglob('*.idat'))
                               + list(Path(sample_sheet_file.parent).rglob('*.idat.gz')))
                instructions.append(f"For {int(len(idats_found) / 2)} {array_type} samples run: "
                                    f"`methylprep process -d {sample_sheet_file.parent} --all`")
    return instructions
