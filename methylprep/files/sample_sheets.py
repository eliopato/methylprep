...
def find_sample_sheet(dir_path, return_all=False, create_if_not_found=True):
    ...
    csv_files = [c for c in sample_dir.rglob('*.csv')]
    ...
    if num_candidates == 0:
        ...
        if len(name_matched) == 1:
            ...
            if return_all:
                ...
                raise Exception(f"Too many sample sheets in this directory. Move or rename redundant ones. "
                                f"Or specify the path to the one to use with --sample_sheet. "
...
def create_sample_sheet(dir_path, matrix_file=False, output_file='samplesheet.csv', sample_type='',
                        sample_sub_type=''):
...
class SampleSheet():
    ...
    def read(self, sample_sheet_file):
        ...
        Dev notes:
            It loads whole file using pandas.read_csv to better handle whitespace/matching on headers.""".format(
            REQUIRED_HEADERS)
        ...
        for idx, row in enumerate(
            test_sheet):  # header is not the first row. alt format is that header begins on row after [Data]
            if rows_to_scan == 0:
                LOGGER.info(f'DEBUG {cur_line} {line_bits}')
                raise ValueError(
                    'Sample sheet is invalid. Could not find start of data row, assuming there should be a [Data] row to start data, and no more than 25 preceding rows.')
